...
{	
	...
	if ($#ARGV >= 0) {
		...
		($inpCmdFile) = '../include/program1.txt';
	}
...
sub readCmd
{
	...
	if ($arrayLength > 0 && ($op ne ""))
	{
		...
		elsif ($op =~ /VSRA/)
		{
			if ($op =~ /VSRAI/)
			{
				&regType($op, '001111', 1, @operands);
			}
			else
			{
				&regType($op, '001110', 0, @operands);
			}
		}
